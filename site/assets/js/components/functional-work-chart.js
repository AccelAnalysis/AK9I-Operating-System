...
export function renderFunctionalWork(){
 ...
 return `<div class="functional-work-chart"><div class="work-top-row">${topCard(reg,'regulatory','Independent assurance • no direct reports')}${topCard(ceo,'ceo','Enterprise direction and final accountability')}${topCard(coo,'coo','Scope Under Resolution')}</div><svg class="work-transition-links" viewBox="0 0 1000 754" preserveAspectRatio="none" aria-hidden="true"><path class="work-coo-admin-link" d="M760 90 C820 120 900 150 950 210"/></svg><div class="work-function-grid">${cards}</div><div class="work-chart-note"><span>Primary view: functional ownership and work scope. The dotted Nigel-to-Admin &amp; Finance relationship remains transitional.</span><button type="button" data-go="s4h">View reporting hierarchy →</button></div></div>`
}
