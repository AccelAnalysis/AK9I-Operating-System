...
function renderBranchNode(role){
 ...
 const assignment=role.multiple?`<div class="team-assignment"><span>Assigned roster</span>${personPicker(role,true)}</div>${renderRoster(role)}`:personPicker(role,true);
 return `<div class="${classes}" data-role-key="${role.key}"><div class="branch-node-card"><div class="branch-node-title">${roleLink(role)}</div>${assignment}${role.status?`<div class="branch-node-status">${esc(role.status)}</div>`:''}${role.coordinationLabel?`<div class="branch-coordination">${esc(role.coordinationLabel)}</div>`:''}</div>${children.length?`<div class="branch-children child-count-${children.length}">${children.map(renderBranchNode).join('')}</div>`:''}</div>`
}
...
export function renderOrg(){
 ...
 return `<div class="org-canvas authority-chart hierarchy-v2">
  <svg class="hierarchy-links" viewBox="0 0 1000 650" preserveAspectRatio="none" aria-hidden="true">
   <path class="authority-line executive-link" d="M342 50 H408"/>
   ...
   <path class="authority-line branch-link" d="M63 252 V270 H125 V294"/>
   <path class="authority-line branch-link" d="M188 252 V270 H375 V294"/>
   <path class="authority-line branch-link" d="M313 252 V270 H625 V294"/>
   <path class="authority-line branch-link" d="M938 252 V270 H875 V294"/>
  </svg>
  <div class="hierarchy-executives">
   <article class="hierarchy-exec-card regulatory"><h3>${roleLink(reg,'top-link')}</h3>${personPicker(reg)}<p>Independent assurance • no direct reports</p></article>
   ...
   <article class="hierarchy-exec-card coo"><h3>${roleLink(coo,'top-link')}</h3>${personPicker(coo)}<p class="scope-status">Scope Under Resolution</p></article>
  </div>
