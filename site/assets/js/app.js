...
function refreshCoordinationReferences(key){
 const role=findOrgPosition(key);if(!role)return;
 const value=getOrgAssignment(role),coordinator=displayValue(role,value);
 document.querySelectorAll(`[data-coordination-from="${key}"]`).forEach(el=>{const template=el.dataset.coordinationTemplate||'{coordinator}';el.textContent=template.split('{coordinator}').join(coordinator)})
}
function refreshAssignment(key){const role=findOrgPosition(key);if(!role)return;const value=getOrgAssignment(role),full=role.multiple?(value.join(', ')||'Vacant / TBD'):String(value||'Vacant / TBD');document.querySelectorAll(`[data-person-picker="${key}"]`).forEach(b=>{if(b.dataset.pickerLabel!=='fixed')b.textContent=displayValue(role,value);b.title=full;b.setAttribute('aria-label',b.dataset.pickerLabel==='fixed'?`Edit roster: ${full}`:full)});document.querySelectorAll(`[data-menu-for="${key}"] [data-multi-person]`).forEach(b=>{const selected=Array.isArray(value)&&value.includes(b.dataset.multiPerson);b.classList.toggle('selected',selected);const check=b.querySelector('.check');if(check)check.textContent=selected?'✓':''});refreshTeamRoster(key,role,value);refreshCoordinationReferences(key)}
