const STAGES=[
 {n:1,title:'Awareness',color:'#2E70B7',owner:'Marketing + Business Development',body:'Capability becomes visible through campaigns, events, partners, advocates, and targeted relationship development.'},
 ...
 {n:5,title:'Loyalty',color:'#168A93',owner:'Contracts & Field Operations + Sales',body:'Service outcomes, satisfaction, renewal, added sites or services, recurring engagement, and measurable account growth.'},
...
export function renderContractLane(){
 return `<div class="contract-lane"><div class="contract-lane-stages">${STAGES.map(s=>`<article class="contract-stage" style="--lane-color:${s.color}"><div class="contract-stage-number">${s.n}</div><h3>${s.title}</h3><p>${s.body}</p><div class="contract-owner"><span>Primary ownership</span><strong>${s.owner}</strong></div></article>`).join('')}</div><div class="contract-enablement"><div class="contract-enablement-title">Functions that enable every stage</div>${ENABLEMENT.map(([t,d])=>`<div class="contract-enable-item"><strong>${t}</strong><span>${d}</span></div>`).join('')}</div><div class="contract-lane-note"><button type="button" data-go="s13">← Student / Education pipeline visual</button><span>The customer workflow changes by lane; personnel reporting authority remains defined by the organizational chart.</span></div></div>`
}
